...
```
![jni编译Java源文件]({{ basepath }}/assets/images/android/jni编译Java源文件.png)
...
![jni执行结果.png]({{ basepath }}/assets/images/android/jni执行结果.png)
...
类的签名比较简单，它采用`L+包名+类名+;`的形式，只需要将其中的"."替换为"/"即可，比如  
`java.lang.String`的签名为`Ljava/lang/System;`，末尾的";"也是签名的一部分。
