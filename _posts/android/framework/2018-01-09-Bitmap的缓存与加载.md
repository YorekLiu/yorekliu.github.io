...
>  
> 2.3.3 (API level 10)以及更低版本，像素数据存储在native内存上；而Bitmap对象存储在Dalvik堆上。像素数据没有以一种可预测的方式来释放，所以需要我们手动`release`。  
> 3.0 (API level 11) 到 Android 7.1 (API level 25)，像素数据和Bitmap对象都存储在Dalvik堆上。可以不动手动`release`了。  
