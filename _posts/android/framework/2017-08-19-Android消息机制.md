...
Talk is cheap, show me the code. 我们知道`LocalThread`的`get/set/remove`方法都是调用的`LocalThreadMap`的对应方法。那么，我们一个个来看：
...
从`ThreadLocal`的`get/set/remove`可以看出，它们所操作的对象都是当前线程的`threadLocals`对象的`Entry[] table`数组，Entry是保存着ThreadLocal与对应value的键值对。因此，不同线程访问同一个`ThreadLocal`，实际上是访问的自己线程内部的以`ThreadLocal`为key的value，这就是`ThreadLocal`可以在多个线程中互不干扰地存储和修改数据的原理了。
