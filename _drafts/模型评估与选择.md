...
查准率与查全率是一对矛盾的存在。一般来说，查准率高时，查全率往往偏低。查全率高时，查准率往往偏低。

在很多情形下，我们可根据学习器的预测结果对样例进行排序，排在前面的是学习器认为最可能是正例的样本，排在最后的则是学习器认为最不可能是正例的样本。

以查准率为纵轴、查全率为横轴，可以得到P-R曲线，显示该曲线的图为P-R图。

![ml_p_r]({{ basepath }}/assets/images/ml/ml_p_r.png)

当P=R时，此时的点称为平衡点(Break-Even Point, BEP)。平衡点越大越优。

一般更常用的是$$\mathit{F}1$$度量：

$$
F1=\frac{2 \times P \times R}{P+R}=\frac{2 \times TP}{样例总数+TP-TN}
$$

$$\mathit{F}1$$度量的一般形式——$$\mathit{F_\beta}$$，可以表达出对查准率、查全率的不同偏好：
...
在n个二分类混淆矩阵上综合考察查准率与查全率：
- 若先在各混淆矩阵上分别计算，然后在计算平均值，这样称之为“宏查准率($$macro⎼P$$)”、“宏查全率($$macro⎼R$$)”以及”宏$$F1(macro⎼F1)$$”
