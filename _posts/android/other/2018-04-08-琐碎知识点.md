...
圆形切换动画、CircularReveal

```kotlin
val maxRadius = Math.hypot(mKeyboardView.width.toDouble() / 2, mKeyboardView.height.toDouble() / 2).toFloat()

val animator =
    ViewAnimationUtils.createCircularReveal(
        mKeyboardView,
        mKeyboardView.width / 2,
        mKeyboardView.height / 2,
        0F,
        maxRadius
    )
...
{% raw %}<p id="dynamic-permission"/>{% endraw %}
...
在lib中重命名output并copy到主工程libs文件夹下面
