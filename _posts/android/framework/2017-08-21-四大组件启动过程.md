...
![Service的bind流程]({{ basepath }}/assets/images/Service的bind流程.png)
...
当ContentProvider所在的线程启动时，ContentProvider会同时启动并被发布到AMS中。ContentProvider的`onCreate`方法要先于Application的`onCreate`方法执行。ContentProvider的`onCreate`要优先于`Application`的`onCreate`方法执行。

{% capture fig_img %}
![Content Provider启动流程图]({{ basepath }}/assets/images/contentprovider启动流程图.png)
{% endcapture %}
<figure>
